...
- **Tạo Hồ sơ BM01:** một trang không có tab; cột nội dung đặt khu chọn Giảng viên hướng dẫn trước khu PDF, cột phụ giữ checklist và giải thích trạng thái khóa. Modal chọn GVHD là danh sách một cột, mỗi dòng có thông tin Giảng viên và nút `+` đỏ ở cuối. Nút `Xem trước PDF` mở dialog gần toàn viewport: canvas xám và trang A4 trắng bên trái, form trắng có vùng cuộn riêng bên phải; mobile xếp thành một cột. Action bar chỉ có `Quay lại` và một primary `Nộp PDF & tạo Hồ sơ`. Khi GVHD từ chối ký, màn hình read-only đặt danger notice và card phản hồi bắt buộc trước PDF bất biến; cột phụ giải thích điều kiện, còn CTA `Tạo Hồ sơ thay thế` chỉ hiện khi Đợt còn mở. Các workspace bằng chứng khác vẫn dùng cột nội dung + điều kiện/phiên bản khi nghiệp vụ cho phép.
