...
- **Workspace P.KHCN:** đầu trang là bốn metric card dùng cùng scope với danh sách; tiếp theo là lifecycle ngang có trạng thái hoàn tất/hiện tại/bị chặn/sắp tới. Mỗi record hiển thị kicker `loại đối tượng · phiên bản · actor chịu trách nhiệm`, một hành động tiếp theo và liên kết tới đúng action group. Official snapshot, phân quyền tài liệu và phiên bản đặt trong detail panel hai cột; audit timeline nằm cuối vùng bằng chứng. Action của P.KHCN không bám đáy thành một dải nút dài mà nhóm theo đối tượng, mỗi nhóm nêu hệ quả và mở decision-dialog có object, role, version, gate/lý do. Ở mobile, metric, lifecycle, detail và action group xếp một cột; không tạo cuộn ngang toàn trang.
